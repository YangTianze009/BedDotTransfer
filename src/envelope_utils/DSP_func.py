...
def wavelet_reconstruction(x, fs, low, high):
    ...
    rec_a, rec_d = wavelet_decomposition(data = x, wave = 'db12', Fs = fs,
...
def signal_quality_assessment(x, Fs, n_lag, low, high,
                                 denoised_method = 'bandpass', show = False):
    x = (x - np.mean(x))/np.std(x)
    if denoised_method == 'DWT':
        denoised_sig = wavelet_reconstruction(x = x, fs = Fs, 
                                              low = low, high = high)
