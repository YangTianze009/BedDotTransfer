...
from src.train.datautils import BaseECGDataset
from models.deeplearn import PeakClassifier
from src.train.evaluators import label_evaluate as evaluate
...
class ECGSignal(BaseECGDataset):
    ...
    def __getitem__(self, idx):
        ...
        peaks = data["pos"][: self._padding] + [0.0] * (
            self._padding - len(data["pos"])
        )
        return (
            tc.tensor(peaks).float() / len(data["data"]),
            data["label"],
        )  # Peak --> Arrithymia
...
def fit(
    ...
    valid = tc.utils.data.DataLoader(
        datas[1], batch_size=batch_size, shuffle=False, num_workers=2, pin_memory=True
    )
    test = tc.utils.data.DataLoader(
        datas[2], batch_size=batch_size, shuffle=False, num_workers=2, pin_memory=True
    )
    ...
    for epoch in range(epoch + 1, num_epochs + 1):
        ...
        for batch in train:
            ...
            if idx == 1 or idx % log_frequency == 0:
                ...
                print(
                    "Train Step=%d | %s"
                    % (idx, "|".join("%s: %.4f" % _ for _ in scores.items()))
                )
    ...
    print(
        "Test Epoch=%d | %s" % (epoch, "|".join("%s: %.4f" % _ for _ in scores.items()))
    )
    return model
...
if __name__ == "__main__":
    ...
    datas = [
        ECGSignal(root + "/" + subset, subset, train=0.85, valid=0.05)
        for subset in ["train", "valid", "test"]
    ]
    model = PeakClassifier(**MODEL_CONFIG)
